...
6. [Repositórios relacionados](#repositórios-relacionados)
...
* Dependências estão citadas no package.json


## Instalação

`npm install` baixa as dependências do arquivo package.json e cria a pasta node_modules.

`npm start` executa a aplicação em modo de desenvolvimento [http://localhost:3000](http://localhost:3000).

`npm run build` cria um `build` otimizado para produção na pasta `build`.
